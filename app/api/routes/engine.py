...
from app.core.security import require_engine_key
...
@router.post("/start", dependencies=[Depends(require_engine_key)], status_code=202)
...
@router.post("/stop", dependencies=[Depends(require_engine_key)])
...
@router.post("/pause", dependencies=[Depends(require_engine_key)])
...
@router.post("/resume", dependencies=[Depends(require_engine_key)])
...
@router.post("/unlock-risk", dependencies=[Depends(require_engine_key)])
...
@router.get("/config/{engine_id}", dependencies=[Depends(require_engine_key)])
async def get_config(engine_id: str, manager: EngineManager = Depends(engines)):
    try: return asdict(manager.get(engine_id).config)
    except KeyError as exc: raise _error(exc) from exc


@router.patch("/config/{engine_id}", dependencies=[Depends(require_engine_key)])
